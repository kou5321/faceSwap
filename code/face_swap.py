...
def bilinear_interpolate(img, coords):
    ...
    @return: array of interpolated pixels with same shape as coords
    """
...
def apply_mask(img, mask):
    ...
    @return: new image with mask applied
    """
...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img, args, end=48):
    ...
    ### imshow
    cv2.namedWindow("warped_src_face", cv2.WINDOW_NORMAL)
    cv2.imshow("warped_src_face", warped_src_face)
    filename = './temp/warped_src_face.jpg'
    cv2.imwrite(filename, warped_src_face)
    cv2.waitKey(0)
    ...
    for i in range(mask_cp.shape[0]):
        for j in range(mask_cp.shape[1]):
            if (mask_cp[i][j] == 255):
                mask_cp[i][j] = 1
    imgWithMask = IP.rgb2gray(dst_face) * mask_cp
    imgWithMask = cv2.convertScaleAbs(imgWithMask, alpha=(255.0))
    cv2.namedWindow("img_copy", cv2.WINDOW_NORMAL)
    cv2.imshow("img_copy", imgWithMask)
    filename = './temp/temp.jpg'
    cv2.imwrite(filename, imgWithMask)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ...
    ### imshow erodemask
    erodeMask = mask.copy()
    for i in range(erodeMask.shape[0]):
        for j in range(erodeMask.shape[1]):
            if (erodeMask[i][j] == 255):
                erodeMask[i][j] = 1
    erodeMask = IP.rgb2gray(dst_face) * erodeMask
    erodeMask = cv2.convertScaleAbs(erodeMask, alpha=(255.0))
    cv2.namedWindow("img_copy", cv2.WINDOW_NORMAL)
    cv2.imshow("img_copy", erodeMask)
    filename = './temp/erodeMask.jpg'
    cv2.imwrite(filename, erodeMask)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ##Poisson Blending
    r = cv2.boundingRect(mask) # Use a minimal rectangle and wrap the found shape
