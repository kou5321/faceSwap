...
PREDICTOR_PATH = 'models/shape_predictor_68_face_landmarks.dat' # facial landmarks recognition
...
def select_face(im, r=10, choose=True):
    ...
    if len(faces) == 1 or not choose:
        ...
        for face in faces:
            ...
            a = cv2.rectangle(im_copy, (face.left(), face.top()), (face.right(), face.bottom()), (0, 0, 255), 1)
        cv2.namedWindow("img_copy", cv2.WINDOW_NORMAL)
        cv2.imshow("img_copy", a)
        filename = './temp/src_bounding.jpg'
...
def select_all_faces(im, r=10):
    ...
    for i, bbox in enumerate(faces):
        ...
        cv2.namedWindow("img_copy", cv2.WINDOW_NORMAL)
        cv2.imshow("img_copy", a)
        filename = './temp/dst_bounding.jpg'
        cv2.imwrite(filename, a)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
if __name__ == '__main__':
    src_img = cv2.imread(r'imgs/test6.jpg')
    # # detect positions of faces
    ...
    dst_img = cv2.imread(r'imgs/mask.jpg')
    select_all_faces = select_all_faces(dst_img)